class OrganizersController < ApplicationController
  ...
  before_action :require_super_admin, only: %i[approve reject destroy]
  before_action :set_organizer, only: %i[approve reject destroy]
  ...
  def destroy
    if @organizer.destroy
      redirect_to organizers_path, notice: "Organizer removed."
    else
      redirect_to organizers_path, alert: @organizer.errors.full_messages.to_sentence
    end
  end
