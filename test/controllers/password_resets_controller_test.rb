...
class PasswordResetsControllerTest < ActionDispatch::IntegrationTest
  test "requesting a reset for a known email enqueues an email without revealing whether it exists" do
    user = User.create!(name: "Demo Parent", email: "reset-parent@example.com", password: "password123", role: :parent)

    assert_enqueued_email_with PasswordMailer, :reset_instructions, params: { user: user } do
      post password_reset_path, params: { email: "reset-parent@example.com" }
    end
  ...
  test "a valid token lets the user set a new password and sign in with it" do
    user = User.create!(name: "Demo Parent", email: "reset-flow-parent@example.com", password: "password123", role: :parent)
    ...
    post login_path, params: { email: "reset-flow-parent@example.com", password: "newpassword456" }
  ...
  test "a token becomes invalid once the password has already been changed" do
    user = User.create!(name: "Demo Parent", email: "reused-token-parent@example.com", password: "password123", role: :parent)
    token = user.generate_token_for(:password_reset)
  ...
  test "mismatched password confirmation shows a validation error" do
    user = User.create!(name: "Demo Parent", email: "mismatch-parent@example.com", password: "password123", role: :parent)
    token = user.generate_token_for(:password_reset)
  ...
  test "already signed in users are redirected away from the reset flow" do
    user = User.create!(name: "Demo Parent", email: "signed-in-reset-parent@example.com", password: "password123", role: :parent)
    sign_in_as user
