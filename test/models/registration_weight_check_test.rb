...
class RegistrationWeightCheckTest < ActiveSupport::TestCase
  ...
  test "weight checks are locked once the tournament is cancelled" do
    organizer = User.create!(name: "Organizer", email: "cancelled-weight-organizer@example.test", password: "password123", role: :organizer)
    tournament = Tournament.create!(name: "Cancelled Weight Open", organizer: organizer, start_date: Date.new(2026, 12, 5), end_date: Date.new(2026, 12, 6))
    category = tournament.tournament_categories.find_or_create_by!(event_type: "kyorugi", gender: "male", age_min: 18, weight_max: 80)

    parent = User.create!(name: "Parent", email: "cancelled-weight-parent@example.test", password: "password123", role: :parent)
    athlete = parent.athletes.create!(first_name: "Cancelled", last_name: "Athlete", date_of_birth: Date.new(2005, 1, 1), gender: "male")
    registration = Registration.create!(
      tournament: tournament,
      athlete: athlete,
      tournament_category: category,
    ...
    assert_not weight_check.valid?
    assert_includes weight_check.errors[:base], "Weight check is locked because the tournament has been cancelled"
  end
end
